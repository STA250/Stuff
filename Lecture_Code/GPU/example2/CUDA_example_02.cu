...
int main (int argc, char *argv[])
{
    ...
    if (stat != CUBLAS_STATUS_SUCCESS) {
        ...
        if (stat == CUBLAS_STATUS_ALLOC_FAILED){
        	printf("CUBLAS allocation failed!");
        }
        return EXIT_FAILURE;
    } else {
    	printf("CUBLAS initialization success...\n");
    }

    int cublas_version;
	stat = cublasGetVersion(handle, &cublas_version);
	if (stat == CUBLAS_STATUS_SUCCESS){
		printf("CUBLAS version: %d\n",cublas_version);
	} else {
		printf("Unable to retrieve CUBLAS version.\n");
	}
	...
	stat = cublasSetVector(N, sizeof(float), arg, 1, gpu_arg, 1);
	if (stat){
	...
	stat = cublasGetVector(N, sizeof(float), gpu_res, 1, res, 1);
	...
	float int_result = 0.0;
	stat = cublasSasum(handle, N, gpu_res, 1, &int_result);
	int_result = int_result*((float)((double)TWO_PI/(double)N)); // * delta

	// TODO: Make delta vector and then do trapezoid rule...?
	...
	if (stat == CUBLAS_STATUS_SUCCESS){
		printf("CUBLAS shutdown successful...\n");
	} else {
